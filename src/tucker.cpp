...
void transpose(int n1, int n2, int n3, REAL* a, int dim)
{
...
//	std::cout << "Zero constructor called" << std::endl;
	...
	r2 = r2_;
	r3 = r3_;
	g = new REAL[r1 * r2 * r3](); // () IMPORTANT
	u1 = new REAL[n1 * r1];
	u2 = new REAL[n2 * r2];
	u3 = new REAL[n3 * r3];
}
// Compress a tensor with given accuracy
Tucker::Tucker(int n1_, int n2_, int n3_, REAL *a, REAL eps)
: n1(n1_), n2(n2_), n3(n3_)
{
...
Tucker::Tucker(int n1_, int n2_, int n3_, REAL *u1_, REAL *u2_, REAL *u3_)
: n1(n1_), n2(n2_), n3(n3_)
{
	r1 = 1;
	r2 = 1;
...
REAL Tucker::compression() const
{
	REAL compression =
			static_cast<REAL>(r1 * r2 * r3 + r1 * n1 + r2 * n2 + r3 * n3) /
			static_cast<REAL>(n1 * n2 * n3);
	return compression;
}
// Print tensor
...
// Get element
REAL Tucker::At(int i1, int i2, int i3) const
{
	REAL a = 0.0;
...
void Tucker::orthogonalize()
{
	...
	REAL **QR1 = qr(n1, r1, u1);
	REAL **QR2 = qr(n2, r2, u2);
	REAL **QR3 = qr(n3, r3, u3);
	...
	REAL *z3 = new REAL[r3*std::min(n1, r1)*std::min(n2, r2)];
...
// Recompress tensor
void Tucker::round(REAL tol, int rmax)
{
	const REAL alpha = 1.0;
	const REAL beta = 0.0;
	...
	REAL *u1_tmp = new REAL[n1 * r1_tmp];
	cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans,
	...
	REAL *u2_tmp = new REAL[n2 * r2_tmp];
	cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans,
	...
	REAL *u3_tmp = new REAL[n3 * r3_tmp];
	cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans,
...
Tucker operator *(const REAL alpha, const Tucker& t)
{
	Tucker res(t);	

	MKL_Dimatcopy('R', 'N', 1, t.r1 * t.r2 * t.r3, alpha, res.g, t.r1 * t.r2 * t.r3, t.r1 * t.r2 * t.r3); // TODO figure out

	return res;
}

Tucker operator *(const Tucker& t, const REAL alpha)
{
	return alpha * t;
}
...
Tucker operator /(const Tucker& t1, const Tucker& t2)
{
	...
	MKL_Dimatcopy('R', 'N', 1, t1.r1 * t1.r2 * t1.r3, 1.0 / t2.g[0], t.g, t.r1 * t.r2 * t.r3, t1.r1 * t1.r2 * t1.r3);
	for (int i = 0; i < t1.n1; ++i){
		MKL_Dimatcopy('R', 'N', 1, t1.r1, 1.0 / t2.u1[i], t.u1 + i * t1.r1, t1.r1, t1.r1);
	}
	for (int i = 0; i < t1.n2; ++i){
		MKL_Dimatcopy('R', 'N', 1, t1.r2, 1.0 / t2.u2[i], t.u2 + i * t1.r2, t1.r2, t1.r2);
	}
	for (int i = 0; i < t1.n3; ++i){
		MKL_Dimatcopy('R', 'N', 1, t1.r3, 1.0 / t2.u3[i], t.u3 + i * t1.r3, t1.r3, t1.r3);
	}
...
REAL *svd_trunc(int m, int n, REAL *a, REAL eps, int &r)
{
	...
	REAL *superb = new REAL[std::min(m,n)-1];

	REAL *a_copy = new REAL[m*n];
...
REAL *svd_trunc_rmax(int m, int n, REAL *a, int rmax)
{
	...
	REAL *u = new REAL[m*rmax]; // TODO can be optimized
...
{
	REAL **result = new REAL *[4]; //TODO: tuple
	...
	REAL *z1 = new REAL[n1*n2*n3];
	REAL *z2 = new REAL[n1*n2*n3];
	REAL *z3 = new REAL[n1*n2*n3];
...
REAL **qr(int n1, int n2, const REAL *a)
{
